...
class SSHApp:
    def __init__(self, root):
        ...
        # Путь для локального сохранения метрик
        ...
        # Путь для директории Zabbix данных
    ...
    def connect_ssh(self):
        ...
        # Проверяем, указан ли volume
        if "/var/lib/docker/volumes" in zabbix_data_path:
            zabbix_volume_name = os.path.basename(zabbix_data_path)
        else:
            zabbix_volume_name = None

        if not ip or not username or not password or not zabbix_volume_name:
            messagebox.showerror("Ошибка",
                                 "Пожалуйста, заполните все обязательные поля и убедитесь, что выбран volume!")
            return
        ...
        try:
            ssh_connection(ip, username, password, local_backup_path, local_metric_path, zabbix_volume_name)
